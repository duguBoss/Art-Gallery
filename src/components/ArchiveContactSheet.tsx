...
export const ArchiveContactSheet: React.FC<ArchiveContactSheetProps> = ({
  ...
  const filterOptions = [
    ...
    { id: 'cyber', label: t('archive.filterCyber') },
    { id: 'brutalist', label: t('archive.filterBrutalist') },
    { id: 'editorial', label: t('archive.filterEditorial') },
    { id: 'zen', label: t('archive.filterZen') },
    { id: 'ghibli', label: t('archive.filterGhibli') },
  ];

  const filteredScenes = scenes.filter((s) => {
    if (filter === 'cyber') return s.id.includes('cyber') || s.cameraRig.mood.toLowerCase().includes('cyber') || s.id.includes('dune');
    if (filter === 'brutalist') return s.id.includes('brutalist') || s.cameraRig.mood.toLowerCase().includes('brutalist') || s.id.includes('kubrick');
    if (filter === 'editorial') return s.id.includes('editorial') || s.cameraRig.mood.toLowerCase().includes('editorial') || s.id.includes('tarkovsky');
    if (filter === 'zen') return s.id.includes('zen') || s.cameraRig.mood.toLowerCase().includes('zen') || s.id.includes('wkw');
    if (filter === 'ghibli') return s.id.includes('ghibli') || s.cameraRig.mood.toLowerCase().includes('ghibli') || s.id.includes('chrome');
    return true;
  }).sort((a, b) => {
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-[#F2F0E8]/10 pb-6 mb-8">
        <div>
          ...
          <p className="text-xs text-[#8B887F] font-mono mt-1">
            {t('archive.status', { count: filteredScenes.length, total: scenes.length })}
          </p>
        </div>

        {/* Filter Strip */}
        <div className="flex flex-wrap items-center gap-2 text-xs font-mono">
          <span className="text-[#8B887F] text-[10px] mr-1 hidden sm:inline">{t('archive.filterLabel')}</span>
            ...
            >
              ...
              <div className="relative aspect-[16/10] bg-[#181815] border border-[#F2F0E8]/10 mt-2 overflow-hidden">
                ...
                <div className="absolute inset-0 flex flex-col justify-between p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none">
                  ...
                  <div>
                    <div className="text-xs font-mono font-bold text-white uppercase">{displayTitle}</div>

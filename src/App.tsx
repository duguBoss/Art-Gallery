...
function AppContent() {
  ...
  useAtlasRouter(lang, currentTab === 'language' ? 'constellation' : (currentTab as any), selectedSceneId, (route) => {
  ...
  const handleToggleDossier = (scene: CinemaScene) => {
    playSpotlightClick();
    setSavedSceneIds((prev) => prev.includes(scene.id) ? prev.filter((id) => id !== scene.id) : [...prev, scene.id]);
  };
  ...
  return (
    <div className="min-h-screen w-full bg-[#11110F] text-[#F2F0E8] font-sans flex flex-col justify-between selection:bg-[#D8FF3E] selection:text-[#11110F]">
      ...
      <main className="flex-1 w-full">
        {selectedSceneId ? (
          <SceneDetailView scene={activeScene} allScenes={scenes} onBack={() => go('archive')} onSelectScene={(id) => { setSelectedSceneId(id); window.scrollTo({ top: 0, behavior: 'smooth' }); }} onExploreTag={() => go('archive')} onOpenInLab={() => go('lab')} onSaveToDossier={handleToggleDossier} isSavedInDossier={savedSceneIds.includes(activeScene.id)} />
        ) : (
          <>
            {currentTab === 'index' && <OpeningSequenceView featuredScene={scenes[0]} totalScenesCount={scenes.length} onStudyScene={(id) => { setSelectedSceneId(id); window.scrollTo({ top: 0, behavior: 'smooth' }); }} onExploreArchive={() => go('archive')} />}
            {currentTab === 'archive' && <ArchiveContactSheet scenes={scenes} onSelectScene={(id) => { setSelectedSceneId(id); window.scrollTo({ top: 0, behavior: 'smooth' }); }} />}
            {currentTab === 'knowledge' && <KnowledgeHubView />}
            {currentTab === 'language' && <VisualConstellationView scenes={scenes} onSelectScene={(id) => { setSelectedSceneId(id); window.scrollTo({ top: 0, behavior: 'smooth' }); }} />}
            {currentTab === 'dossiers' && <DossiersView savedSceneIds={savedSceneIds} allScenes={scenes} onSelectScene={(id) => { setSelectedSceneId(id); window.scrollTo({ top: 0, behavior: 'smooth' }); }} onRemoveFromDossier={handleRemoveFromDossier} />}
            ...
            {currentTab === 'tools' && <KnowledgeHubView />}
      ...
      <CommandPalette isOpen={isCommandOpen} onClose={() => setIsCommandOpen(false)} scenes={scenes} onSelectScene={(id) => { setSelectedSceneId(id); window.scrollTo({ top: 0, behavior: 'smooth' }); }} onNavigateTab={(tab) => { setSelectedSceneId(null); setCurrentTab(tab); window.scrollTo({ top: 0, behavior: 'smooth' }); }} />

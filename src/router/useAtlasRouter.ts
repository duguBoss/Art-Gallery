...
export function parseLocation(): Partial<AtlasRouteState> {
  const match=window.location.pathname.match(/\/knowledge\/([^/]+)\/?$/); if(match)return{tab:'knowledge',knowledgeId:decodeURIComponent(match[1])};
  return parseHash(window.location.hash);
}

export function buildRouteUrl(state:{locale:Language;tab:AtlasRouteTab;sceneId?:string;atomId?:string;knowledgeId?:string}){
  const loc=state.locale==='zh'?'zh-CN':state.locale; const base=import.meta.env.BASE_URL.replace(/\/$/,'');
  if(state.knowledgeId&&state.tab==='knowledge')return `${base}/knowledge/${encodeURIComponent(state.knowledgeId)}/`;
  if(state.sceneId&&state.tab==='index')return `#/${loc}/scene/${state.sceneId}`; if(state.atomId&&state.tab==='constellation')return `#/${loc}/constellation/${state.atomId}`; return `#/${loc}/${state.tab}`;
}
...
export function useAtlasRouter(currentLocale:Language,currentTab:AtlasRouteTab,currentSceneId:string|null,currentKnowledgeId:string|null,onNavigate:(state:Partial<AtlasRouteState>)=>void){
  const sync=useCallback(()=>{const target=buildRouteUrl({locale:currentLocale,tab:currentTab,sceneId:currentSceneId||undefined,knowledgeId:currentKnowledgeId||undefined}); if(target.startsWith('#')){if(window.location.hash!==target)window.history.replaceState(null,'',target)}else if(window.location.pathname!==target){window.history.replaceState(null,'',target)}},[currentLocale,currentTab,currentSceneId,currentKnowledgeId]);
  useEffect(()=>{sync()},[sync]);
  useEffect(()=>{const h=()=>onNavigate(parseLocation());window.addEventListener('hashchange',h);if(window.location.hash||window.location.pathname.includes('/knowledge/'))h();return()=>window.removeEventListener('hashchange',h)},[onNavigate]);
}

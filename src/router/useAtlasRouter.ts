...
export interface AtlasRouteState { locale: Language; tab: AtlasRouteTab; sceneId?: string; atomId?: string; }
...
export function parseHash(hash: string): Partial<AtlasRouteState> {
  ...
  const tab = tabCandidate && validTabs.includes(tabCandidate) ? tabCandidate : 'index';
  const param = parts[tabIndex + 1];
  return { locale, tab, sceneId: tab === 'index' && param?.startsWith('scene-') ? param : undefined, atomId: tab === 'constellation' && param ? param : undefined };
}
...
export function useAtlasRouter(currentLocale: Language, currentTab: AtlasRouteTab, currentSceneId: string | null, onNavigate: (state: Partial<AtlasRouteState>) => void) {
  const syncToHash = useCallback(() => {
    const hash = buildRouteHash({ locale: currentLocale, tab: currentTab, sceneId: currentSceneId || undefined });
    if (window.location.hash !== hash) window.history.replaceState(null, '', hash);
  }, [currentLocale, currentTab, currentSceneId]);
  useEffect(() => { syncToHash(); }, [syncToHash]);
  useEffect(() => {
    const handleHashChange = () => onNavigate(parseHash(window.location.hash));
